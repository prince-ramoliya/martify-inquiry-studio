import { Link } from "react-router-dom";
import { Button } from "./ui/button";
import { ProductCard } from "./ProductCard";
import { ProductRow } from "./ProductRow";
import { featuredProducts, products } from "@/data/products";

// Curated lists (deterministic — by rating / badges)
const trendingProducts = products
  .filter((p) => p.badge === "Trending" || p.badge === "Hot" || (p.rating ?? 0) >= 4.7)
  .slice(0, 10);

const bestSellers = [...products]
  .sort((a, b) => (b.rating ?? 0) * (b.reviewCount ?? 0) - (a.rating ?? 0) * (a.reviewCount ?? 0))
  .slice(0, 10);

const newArrivalsList = products.filter((p) => p.isNew).slice(0, 10);

export const FeaturedProducts = () => {
  return (
    <>
      {/* MOBILE: horizontal-scroll sections */}
      <div className="md:hidden bg-gradient-soft pt-6 pb-4">
        <ProductRow
          eyebrow="Trending now"
          title="Trending Products"
          products={trendingProducts}
        />
        <ProductRow
          eyebrow="Top picks"
          title="This Week's Best-Sellers"
          products={bestSellers}
        />
        {newArrivalsList.length > 0 && (
          <ProductRow
            eyebrow="Just landed"
            title="New Arrivals"
            products={newArrivalsList}
          />
        )}
      </div>

      {/* DESKTOP: original grid */}
      <section className="hidden md:block py-20 lg:py-28 bg-gradient-soft">
        <div className="container-page">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
            <div className="max-w-xl">
              <div className="text-sm font-semibold text-primary uppercase tracking-wider mb-3">This week's picks</div>
              <h2 className="font-display font-bold text-3xl lg:text-5xl tracking-tight">
                Hand-selected by our team
              </h2>
            </div>
            <p className="text-muted-foreground max-w-md">
              Every Monday we refresh this list with products our customers are loving and asking us to restock.
            </p>
          </div>

          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6">
            {featuredProducts.slice(0, 8).map((p, i) => (
              <ProductCard key={p.id} p={p} i={i} />
            ))}
          </div>

          <div className="mt-12 text-center">
            <Link to="/shop">
              <Button variant="outline" size="lg">View all products</Button>
            </Link>
          </div>
        </div>

        {/* Desktop: extra horizontal-scroll trending row */}
        <ProductRow
          eyebrow="Trending now"
          title="Trending Products"
          subtitle="What customers are loving and reordering this week."
          products={trendingProducts}
        />
        <ProductRow
          eyebrow="Top picks"
          title="This Week's Best-Sellers"
          subtitle="Tried, tested and rated 4.7+ by 2,400+ shoppers."
          products={bestSellers}
        />
      </section>
    </>
  );
};
